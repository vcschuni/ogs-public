-----------------------------------------------------------------------------
--
-- Create 2 new users that have read and read/write access to the gisdata database
--
-----------------------------------------------------------------------------

-- Create read-only user
CREATE USER ogs_ro_user WITH PASSWORD 'pass123!';

-- Grant read-only permissions
GRANT CONNECT ON DATABASE gisdata TO ogs_ro_user;
GRANT USAGE ON SCHEMA public TO ogs_ro_user;
GRANT SELECT ON ALL TABLES IN SCHEMA public TO ogs_ro_user;

-- Ensure read-only user can access future tables automatically
ALTER DEFAULT PRIVILEGES IN SCHEMA public
GRANT SELECT ON TABLES TO ogs_ro_user;

-- Create read-write user
CREATE USER ogs_rw_user WITH PASSWORD 'pass456!';

-- Grant read-write permissions
GRANT CONNECT ON DATABASE gisdata TO ogs_rw_user;
GRANT USAGE ON SCHEMA public TO ogs_rw_user;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO ogs_rw_user;

-- Ensure read-write user can access future tables automatically
ALTER DEFAULT PRIVILEGES IN SCHEMA public
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO ogs_rw_user;
